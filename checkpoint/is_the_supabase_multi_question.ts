...
export async function saveSurveyResponses(userId: string, responses: { questionId: string, answer: string }[]): Promise<boolean> {
  try {
    ...
    // Convert array of responses to object with question IDs as keys
    const responseObject: Record<string, string> = {};
    responses.forEach(response => {
      responseObject[response.questionId] = response.answer;
    });

    // Add user_id to the response object
    const dataToUpsert = {
      user_id: userId,
      ...responseObject
    };

    // Upsert the responses (insert if not exists, update if exists)
